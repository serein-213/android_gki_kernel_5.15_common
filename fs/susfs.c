...
void susfs_set_uname(void __user **user_info) {
	...
	if (!strcmp(info.release, "default")) {
		strscpy(my_uname.release, utsname()->release, __NEW_UTS_LEN + 1);
	} else {
		strscpy(my_uname.release, info.release, __NEW_UTS_LEN + 1);
	}
	if (!strcmp(info.version, "default")) {
		strscpy(my_uname.version, utsname()->version, __NEW_UTS_LEN + 1);
	} else {
		strscpy(my_uname.version, info.version, __NEW_UTS_LEN + 1);
	}
...
void susfs_spoof_uname(struct new_utsname* tmp) {
	...
	do {
		...
		if (is_susfs_uname_set) {
			strscpy(tmp->release, my_uname.release, __NEW_UTS_LEN + 1);
			strscpy(tmp->version, my_uname.version, __NEW_UTS_LEN + 1);
		}
...
out_add_new_entry:
	...
	strscpy(new_entry_redirected->info.target_pathname, info.redirected_pathname, SUSFS_MAX_LEN_PATHNAME);
	strscpy(new_entry_redirected->info.redirected_pathname, info.target_pathname, SUSFS_MAX_LEN_PATHNAME);
...
int susfs_open_redirect_spoof_do_proc_readlink(struct inode *inode, char *tmp_buf, int buflen) {
	...
	hash_for_each_possible_rcu(OPEN_REDIRECT_HLIST, entry, node, inode->i_ino) {
			...
			entry->target_dev == inode->i_sb->s_dev)
		{
			...
			strscpy(tmp_buf, entry->info.redirected_pathname, buflen);
			srcu_read_unlock(&susfs_srcu_open_redirect, srcu_idx);
			return 0;
		}
	}
...
static int copy_config_to_buf(const char *config_string, char *buf_ptr, size_t *copied_size, size_t bufsize) {
	...
	*copied_size += tmp_size;
	...
	strscpy(buf_ptr, config_string, tmp_size + 1);
	return 0;
}
